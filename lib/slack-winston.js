...
var Slack = (exports.Slack = function(options) {
  ...
  if (!options.webhook_url) {
    if (!options.domain) {
      throw new Error('Must have a domain or webhook_url option set.');
    }
  }
...
Slack.prototype._request = function(options, callback) {
  ...
  var text = this.options.attachments ? options.params.message : options.params.message + (options.params.meta ? '\n\n' + options.params.meta : '');
  var attachments = _.isFunction(this.options.attachments) ? this.options.attachments(options.params, meta) : this.options.attachments;
  ...
  if (!this.options.webhook_url) {
    options.url = util.format(
      'https://%s.slack.com/services/hooks/incoming-webhook',
      this.options.domain
    );
  } else {
